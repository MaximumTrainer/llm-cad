...
- N1 Sandbox: model code runs in a subprocess under two layers of containment.
  - **OS-enforced (hard guarantees).** Wall-clock timeout (30s default, `CAD_MCP_SANDBOX_TIMEOUT_S`); address-space/memory cap (2GB default, `CAD_MCP_SANDBOX_MEM_MB`) via `RLIMIT_AS`/`RLIMIT_DATA` on Unix and a Job Object with `JOB_OBJECT_LIMIT_PROCESS_MEMORY` on Windows; file-size cap (`RLIMIT_FSIZE`); and a process-group/job kill on timeout so nothing is orphaned. These apply to the **pre-warmed** worker as well as a freshly spawned one: the worker a request actually runs on is usually one started at server start-up, so a limit applied only on the cold path is a limit that never applies in production. On Unix the reused worker lowers its own `RLIMIT_AS`/`RLIMIT_DATA` to the caller's figure when it picks up the job; on Windows the Job Object is fixed when the worker is created, because Windows accepts a job assignment onto a running process and then does not enforce it, so a request asking for a *tighter* cap than `CAD_MCP_SANDBOX_MEM_MB` gets a cold worker rather than a silently weaker limit. A process-count brake is also set (`RLIMIT_NPROC` / `ActiveProcessLimit`, `CAD_MCP_SANDBOX_MAX_PROCS`), but it is deliberately generous and is **not** a precise per-execution cap: on Linux `RLIMIT_NPROC` is per real UID and counts threads, and NumPy/OpenBLAS and OCCT each start a pool sized from the CPU count. It scales with `os.cpu_count()` and defaults to at least 256; a flat 64 killed the worker during `import numpy` before any user code ran. Treat it as a runaway-spawn brake — the hard per-execution guarantees are the timeout, the memory cap and the process-group kill.
